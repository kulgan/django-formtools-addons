...
from formtools_addons.wizard.views import (
    MultipleFormWizardView, SessionMultipleFormWizardView, CookieMultipleFormWizardView)
...
class Step6Model(forms.ModelForm):
    class Meta:
        model = TestModel
        fields = ['name']


class Step7Model(forms.ModelForm):
    class Meta:
        model = TestModel
        fields = ['name']


class Step8Model(forms.ModelForm):
    class Meta:
        model = TestModel
        fields = ['name']
...
class TestWizard(MultipleFormWizardView):
...
class FormTests(TestCase):
    ...
    def test_formset_instance(self):
        ...
        self.assertEqual(form.initial_form_count(), 1)

    def test_multipleforms_instance(self):
        request = get_request()
        instance_1 = TestModel.objects.create(name='test_instance_1')
        instance_2 = TestModel.objects.create(name='test_instance_2')
        instance_3 = TestModel.objects.create(name='test_instance_3')
        instance_4 = TestModel.objects.create(name='test_instance_4')
        instance_5 = TestModel.objects.create(name='test_instance_5')
        ...
        testform = TestWizard.as_view(form_list, instance_dict={
            'step1': {
                'substep_1': instance_1,
                'substep_2': instance_2
            },
            'step2': {
                'substep_1': instance_3,
                'substep_2': instance_4
            },
            'step3': instance_5
        })
        response, instance = testform(request)

        self.assertEqual(instance.get_form_instance('step1')['substep_1'], instance_1)
        self.assertEqual(instance.get_form_instance('step1')['substep_2'], instance_2)
        self.assertEqual(instance.get_form_instance('step2')['substep_1'], instance_3)
        self.assertEqual(instance.get_form_instance('step2')['substep_2'], instance_4)
        self.assertEqual(instance.get_form_instance('step3'), instance_5)

    def test_multipleforms_get_forms(self):
        request = get_request()
        instance_1 = TestModel.objects.create(name='test_instance_1')
        instance_2 = TestModel.objects.create(name='test_instance_2')
        instance_3 = TestModel.objects.create(name='test_instance_3')
        ...
        form_list = [
            ('step1', (
                ('substep_1', Step6Model),
                ('substep_2', Step7Model)
            )),
            ('step2', Step8Model)
        ]
        ...
        forms = instance.get_forms('step1')
        self.assertEqual(len(forms), 2)
        self.assertEqual(forms[0].instance, instance_1)
        self.assertEqual(forms[1].instance, instance_2)

        forms = instance.get_forms('step2')
        self.assertEqual(len(forms), 1)
        self.assertEqual(forms[0].instance, instance_3)

        self.assertEqual(instance.get_form_instance('step1')['substep_1'], instance_1)
        self.assertEqual(instance.get_form_instance('step1')['substep_2'], instance_2)
        self.assertEqual(instance.get_form_instance('step2'), instance_3)
...
class SessionFormTests(TestCase):
    def test_init(self):
        request = get_request()
        testform = SessionMultipleFormWizardView.as_view([('start', Step1)])
...
class CookieFormTests(TestCase):
    def test_init(self):
        request = get_request()
        testform = CookieMultipleFormWizardView.as_view([('start', Step1)])
        self.assertIsInstance(testform(request), TemplateResponse)
